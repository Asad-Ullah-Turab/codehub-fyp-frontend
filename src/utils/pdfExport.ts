...
export const exportTutorialToPDF = (tutorial: Tutorial) => {
  ...
  const addFooter = () => {
    ...
    pdf.text("CODEHUB", margin, footerY + 9);
  ...
  pdf.setTextColor(255, 255, 255);
  pdf.setFont("helvetica", "bold");
  pdf.text("CODEHUB", pageWidth / 2, 68, { align: "center" });
  ...
  pdf.setFontSize(11);
  pdf.text(
    `${new Date().getFullYear()} CodeHub Technologies`,
    margin + 18,
    yPosition + 13
  );
  ...
  pdf.setTextColor(colors.accent[0], colors.accent[1], colors.accent[2]);
  pdf.text(
    "www.codehub.com  |  support@codehub.com",
    pageWidth / 2,
    yPosition,
    { align: "center" }
  );
  ...
  const fileName = `CodeHub_${cleanTitle}_${tutorial.language}${
    isAIGenerated ? "_AI" : ""
