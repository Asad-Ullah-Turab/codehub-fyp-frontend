...
export const getAndClearOAuthRedirect = (): string => {
  const redirect = sessionStorage.getItem('oauth_redirect') || ROUTES.EDITOR;
...
export const generateBreadcrumbs = (currentPath: string): Array<{label: string, path: string}> => {
  const breadcrumbs = [];
  ...
  switch (currentPath) {
    ...
    case ROUTES.EMAIL_VERIFICATION:
      breadcrumbs.push({ label: 'Email Verification', path: ROUTES.EMAIL_VERIFICATION });
      break;
    case ROUTES.FORGOT_PASSWORD:
      breadcrumbs.push({ label: 'Forgot Password', path: ROUTES.FORGOT_PASSWORD });
      break;
    case ROUTES.RESET_PASSWORD:
      breadcrumbs.push({ label: 'Reset Password', path: ROUTES.RESET_PASSWORD });
      break;
    default:
